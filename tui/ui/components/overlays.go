...
func (m model) renderSkillsModal() string {
	...
	if len(items) == 0 {
		...
		activeName := ""
		if active := m.activeSkillSnapshot(); active != nil {
			activeName = strings.TrimSpace(active.Name)
		}
...
func (m model) renderActiveSkillBanner() string {
	...
	if len(active.Args) > 0 {
		keys := make([]string, 0, len(active.Args))
		for key := range active.Args {
			keys = append(keys, key)
		}
		sort.Strings(keys)
		pairs := make([]string, 0, len(keys))
		for _, key := range keys {
			pairs = append(pairs, fmt.Sprintf("%s=%s", key, active.Args[key]))
		}
