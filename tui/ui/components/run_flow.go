...
func (m *model) handleAgentEvent(event agent.Event) {
	...
	case agent.EventAssistantMessage:
		...
		if content == "" {
			content = strings.TrimSpace(m.bufferedAssistantText)
		}
		m.finishAssistantMessage(content)
		m.bufferedAssistantText = ""
	...
	case agent.EventRunFinished:
		...
		if strings.TrimSpace(event.Content) == "" && strings.TrimSpace(m.bufferedAssistantText) != "" {
...
func (m model) startRunCmd(runCtx context.Context, runID int, prompt agent.RunPromptInput, mode string) tea.Cmd {
	return func() tea.Msg {
		go func() {
			err := m.runtimeAPI().RunPromptWithInput(runCtx, m.sess, prompt, mode, io.Discard)
